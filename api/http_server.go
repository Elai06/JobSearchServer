...
func (h *HTTPHandler) StartServer() error {
	r := gin.Default()
	r.GET("/auth/authorization", h.authorization)
	r.GET("/auth/callback", h.callback)
	r.GET("/vacancy/response", h.response)
...
func (h *HTTPHandler) authorization(c *gin.Context) {
	authURL := fmt.Sprintf("https://hh.ru/oauth/authorize?response_type=code&client_id=%s&redirect_uri=%s",
		h.hClient.clientId, "http://localhost:8080/auth/callback")
...
func (h *HTTPHandler) refreshAccess(c *gin.Context) {
	resp, err := h.hClient.client.R().SetQueryParams(map[string]string{
		...
		"refresh_token ": h.hClient.tokens.RefreshToken,
...
func (h *HTTPHandler) response(c *gin.Context) {
	for _, vacancy := range h.Vacancies {
		go func() {
			err := h.hClient.ResponceVacancy("d099d869ff0e5a318d0039ed1f797567683538", vacancy.ID)
			if err != nil {
				log.Printf("error responding to vacancy %s : %s", vacancy.ID, err)
			}
		}()
	}
...
func (h *HTTPHandler) searchVacancies(c *gin.Context) {
	...
	if h.hClient.tokens.RefreshToken != "" && h.hClient.tokens.AccessToken != "" {
...
func (h *HTTPHandler) checkIfResponded(vacancies []Vacancy) ([]Vacancy, error) {
	resp, err := h.hClient.client.R().
		SetHeader("Authorization", "Bearer "+h.hClient.tokens.AccessToken).
	...
	for _, negotiation := range result.Items {
		for _, vacancy := range vacancies {
			if vacancy.ID == negotiation.ID {
				continue
			}
